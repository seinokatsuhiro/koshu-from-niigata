...
judgeIO :: Param -> FilePath -> IO K.JudgeC
judgeIO param path =
    do stat <- Posix.getFileStatus path
       return $ judge param path
                  (Posix.fileSize stat)
                  (Posix.modificationTime stat)

judge :: Param -> FilePath -> Posix.COff -> Posix.CTime -> K.JudgeC
judge param path (Posix.COff size) (Posix.CTime time) = K.affirm "FILE" xs where
    xs = [ K.term "time" $ tc
